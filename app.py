        ...
        <article class="panel">
          ...
          <form class="form-card" method="post" action="/commissioner/save">
            ...
            <div class="table-wrap">
              <table>
                <thead><tr><th>Game</th><th>Matchup</th><th>Kickoff</th><th>Spread</th><th>Winner</th><th>Final score</th></tr></thead>
...
def create_week(conn, form):
    ...
    copy_from_current = form.get("copy_from_current") == "1"
    ...
    if copy_from_current and current_week:
        for tb in fetch_week_tiebreakers(conn, current_week["id"]):
            conn.execute(
                "INSERT INTO tiebreakers (week_id, position, prompt) VALUES (?, ?, ?)",
                (new_week_id, tb["position"], tb["prompt"]),
            )
